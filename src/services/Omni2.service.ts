...
@Injectable()
export class Pass2Service {
    constructor(
        @InjectRepository(UsuarioOmnicanal, 'OMNICANAL2') 
        private readonly usuarioRepo: Repository<UsuarioOmnicanal>,
    ) {}

    async actualizarPasswordPorLogin2(login: string, newPassword: string): Promise<void> {
        ...
        if (usuarios.length === 0) {
            throw new Error('No se encontraron usuarios con ese login en OMNICANAL');
        }
